...
return {
    ...
    Description = "Creates a hint visible to everyone. No name is shown with the message.",
    Arguments = {
        {
            Type = "string",
            Name = "Message",
    ...
    ServerRun = function(CommandContext: Types.CmdrCommandContext, Message: string)
        ...
        for Player, FilteredMessage in Api.Filter:FilterStringForPlayers(Message, CommandContext.Executor, Players:GetPlayers()) do
            Api.Messages:DisplayHint(Player, Api.Filter:EscapeRichText(FilteredMessage))
        end
    end,
}