...
--]]
function ApiReference:UpdateApiReference()
    ...
    if not Worked then
        warn("Failed to get latest API version because "..tostring(ApiVersion))
    ...
    local Worked, ApiDumpJson = pcall(function()
        ...
        if Response.StatusCode ~= 200 then
            error("Invalid response returned: "..tostring(Response.StatusCode).." - "..tostring(Response.StatusMessage))
        end
        return Response.Body
    end)
    if not Worked then
        warn("Failed to get lates API dump because "..tostring(ApiDumpJson))
        self.UpdateAttmepted = true
        return
    end

    --Clear the existing StringValues.
    for _, StringValue in pairs(ApiReferenceData:GetChildren()) do
...
--]]
function ApiReference:GetApiDump()
    ...
    if not self.ApiDump then
        ...
        while ApiReferenceData:FindFirstChild(tostring(CurrentIndex)) do
            ApiDumpJson = ApiDumpJson..ApiReferenceData:FindFirstChild(tostring(CurrentIndex)).Value
...
--]]
function ApiReference:GetRaw(ClassName: string)
    for _, Class in pairs(self:GetApiDump().Classes) do
...
--]]
function ApiReference:IsCreateable(ClassName: string): boolean
    local Class = self:GetRaw(ClassName)
    if not Class then return false end
    for _, Tag in pairs(Class.Tags or {}) do
...
--]]
function ApiReference:GetProperties(ClassName: string)
    if not self.PropertiesCache[ClassName] then
        ...
        if Class then
            ...
            if Class.Superclass then
                for _, Property in pairs(self:GetProperties(Class.Superclass)) do
